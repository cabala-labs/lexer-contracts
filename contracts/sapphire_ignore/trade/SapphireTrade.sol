...
contract SapphireTrade is ISapphireTrade, FundWithdrawable {
  ...
  function createPosition(
    ...
    if (collateralToken != _depositToken) {
      ...
      position.totalCollateralAmount = amountOut;
    ...
    position.totalCollateralBalance = position
    ...
    returns (address, uint256)
  {
    ...
    console.log("amount at close:", pnlAmount, feeAmount);
    if (isProfit) {
      ...
      withdrawAmount = position.totalCollateralAmount + pnlAmount - feeAmount;
    } else {
    ...
    return (defaultWithdrawToken, pnlAmount);
  }
  ...
  function closePosition(
    ...
    (address withdrawToken, uint256 withdrawAmount) = _closePosition(_tokenId);

    // swap the asset if needed
    if (_withdrawToken != withdrawToken) {
      withdrawAmount = sapphireSwap.swapToken(
        withdrawToken,
        _withdrawToken,
        withdrawAmount
      );
    ...
    // transfer the token to the _withdrawAddress
  ...
  //   require(_canLiquidate(_tokenId), "cannot liquidate");
  //   // get the owner of the position
  //   address owner = sapphireNFT.ownerOf(_tokenId);
  //   // close position as defaultWithdrawalToken
  //   (address withdrawToken, uint256 withdrawAmount) = _closePosition(_tokenId);
  //   // pay the liquidator fees
  //   uint256 liquidationFee = 0;
  //   // return the remaining fund to the user
  //   IERC20(withdrawToken).transfer(owner, withdrawAmount - liquidationFee);
  // }
}
